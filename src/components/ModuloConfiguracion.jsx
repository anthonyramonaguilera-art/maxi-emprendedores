...
import { Settings, Store, MessageSquare, Save, Loader2, UploadCloud, CheckCircle, CreditCard, DollarSign } from 'lucide-react';
...
export default function ModuloConfiguracion({ usuario }) {
  ...
  const guardarConfiguracion = async (e) => {
    ...
    try {
      // 1. Guardar la Tasa BCV Globalmente
      if (inputTasa) {
        actualizarTasaBcv(inputTasa);
      }
      ...
      // 3. Guardar el Perfil en Supabase
      ...
      alert("¡Configuración y Tasa BCV guardadas con éxito!");
      ...
      <form onSubmit={guardarConfiguracion} className="space-y-6">
        ...
        <div className="bg-emerald-50 p-6 rounded-3xl shadow-sm border border-emerald-100">
          <h3 className="flex items-center gap-2 text-sm font-black text-emerald-800 mb-4 uppercase tracking-wider">
            <DollarSign className="w-5 h-5 text-emerald-600"/> Tasa de Cambio del Día
          </h3>
          ...
          <p className="text-xs text-emerald-600/70 mt-3 font-bold pl-2">Esta tasa calculará automáticamente tus costos y ventas en toda la aplicación.</p>
        </div>
