...
import {
  chatAbierto, historial, eventosPendientesChat, noMolestar, escribiendo,
  toggleChat, cerrarChat, agregarMensaje, limpiarHistorial, setNoMolestar,
  limpiarEventosChat, setEscribiendo
...
export default function MaxiAssistant({ usuario }) {
  ...
  const eventos = useStore(eventosPendientesChat);
  ...
  const handleEnviar = async (textoMensaje) => {
    const texto = (textoMensaje || input).trim();
    if (!texto) return;

    agregarMensaje('user', texto);
    if (!textoMensaje) setInput('');
    setEscribiendo(true);
    ...
    try {
      ...
      } else if (respuesta && respuesta.texto) {
        agregarMensaje('maxi', respuesta.texto, respuesta.opciones);
      }
    } catch (error) {
  ...
  // Enviar un mensaje predefinido (botón de opción)
          ...
          <>
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 bg-slate-50">
                ...
                {mensajes.map((msg, i) => (
                  <div key={i} className={`flex ${msg.tipo === 'user' ? 'justify-end' : 'justify-start flex-col'}`}>
                    ...
                    {/* Botones de opciones (solo para mensajes de Maxi) */}
                    {msg.tipo === 'maxi' && msg.opciones && msg.opciones.length > 0 && (
                      <div className="flex flex-wrap gap-2 mt-2 ml-2">
                        {msg.opciones.map((opcion, idx) => (
                          <button
                            key={idx}
                            onClick={() => handleOpcion(opcion)}
                            className="bg-white border border-indigo-200 text-indigo-600 px-3 py-1 rounded-full text-xs font-bold hover:bg-indigo-50 active:scale-95 transition"
                          >
                            {opcion}
                          </button>
                        ))}
